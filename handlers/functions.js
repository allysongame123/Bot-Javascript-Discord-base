...
function onCoolDown(message, command) {
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           ...
           //go forward
             ...
             //b.reply("***Swapping a PAGE BACK***, *please wait 2 Seconds for the next Input*"'Join in dashboard to config',, true)
               if (currentPage < embeds.length - 1) {
                   currentPage++;
                   await swapmsg.edit({embeds: [embeds[currentPage]], components: allbuttons});
                   await b.deferUpdate();
  ...
  function change_status(client) {
    try {
      ...
      client.user.setActivity(`${config.prefix}help |in ${client.guilds.cache.size} guilds`, {
        type: "STREAMING",
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
